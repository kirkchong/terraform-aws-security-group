...
resource "aws_vpc_security_group_ingress_rule" "ingress_rules_ipv4" {
  count = local.create ? length(local.ingress_rules_cidr_blocks) : 0
  ...
  to_port     = var.rules[local.ingress_rules_cidr_blocks[count.index][0]][1]
  ip_protocol = var.rules[local.ingress_rules_cidr_blocks[count.index][0]][2]
...
resource "aws_vpc_security_group_ingress_rule" "ingress_rules_prefix" {
  count = local.create ? length(local.ingress_rules_prefix_list) : 0

  security_group_id = local.this_sg_id

  prefix_list_id = local.ingress_rules_prefix_list[count.index][1]
  description    = var.rules[local.ingress_rules_prefix_list[count.index][0]][3]

  from_port   = var.rules[local.ingress_rules_prefix_list[count.index][0]][0]
  to_port     = var.rules[local.ingress_rules_prefix_list[count.index][0]][1]
  ip_protocol = var.rules[local.ingress_rules_prefix_list[count.index][0]][2]
...
resource "aws_vpc_security_group_ingress_rule" "computed_ingress_rules_ipv4" {
  count = local.create ? var.number_of_computed_ingress_rules * length(var.ingress_cidr_blocks) : 0
...
resource "aws_vpc_security_group_ingress_rule" "computed_ingress_rules_ipv6" {
  count = local.create ? var.number_of_computed_ingress_rules * length(var.ingress_ipv6_cidr_blocks) : 0
...
resource "aws_vpc_security_group_ingress_rule" "computed_ingress_rules_prefix_list" {
